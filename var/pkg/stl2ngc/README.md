...
make
Note: if you receive someting like ERROR: ld -locl: not found
      edit you make file in order to specify the correct path to opencamlib
sudo make install
```
...
note: if you reseive something like libocl.so.11.* not found
      edit your /etc/ld.so.conf so it can include the path to opencamlib
      them run sudo ldconfig
```
